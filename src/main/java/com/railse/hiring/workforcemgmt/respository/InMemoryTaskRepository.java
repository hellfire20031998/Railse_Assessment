...
public class InMemoryTaskRepository implements TaskRepository {
    ...
    private final Map<Long, List<TaskActivity>> activityLogs = new ConcurrentHashMap<>();
    ...
    public List<TaskManagement> findByTaskPriority(Priority priority) {
        return taskStore.values().stream()
                .filter(task -> task.getPriority().equals(priority))
                .collect(Collectors.toList());
    }

    // This interface extends TaskRepository, which means it inherits all the methods defined in TaskRepository.
    // It can be used to implement an in-memory storage solution for tasks, such as using a HashMap or List.
    // The actual implementation would handle task creation, retrieval, updating, and deletion in memory.

    public void logActivity(Long taskId, String message, String author) {
        ...
        activityLogs.computeIfAbsent(taskId, k -> new ArrayList<>()).add(activity);
    }

    public List<TaskActivity> getActivitiesByTaskId(Long taskId) {
        return activityLogs.getOrDefault(taskId, Collections.emptyList());
    }
    public TaskActivity addActivityLog(Long taskId, TaskActivity activity) {
        long newId = idCounter.incrementAndGet();
    ...
    public TaskComment addComment(Long taskId, String author, String commentText) {
        long newId = idCounter.incrementAndGet();
